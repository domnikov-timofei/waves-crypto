import { createAesKeys } from './aesCommon.js';
...
export function decryptAesEcb(keyInput: Uint8Array, input: Uint8Array) {
  const { decryptionKey } = createAesKeys(keyInput);
  ...
  for (
    ...
    for (let i = 0; i < 4; i++) {
      const kOffset = (56 + i) << 2;

      const key1 = decryptionKey.getUint8(kOffset + 3);
      const si1 = Si[t.getUint8((i << 2) + 3)] ^ key1;

      const key2 = decryptionKey.getUint8(kOffset + 2);
      const si2 = Si[t.getUint8((((i + 3) & 0b11) << 2) + 2)] ^ key2;

      const key3 = decryptionKey.getUint8(kOffset + 1);
      const si3 = Si[t.getUint8((((i + 2) & 0b11) << 2) + 1)] ^ key3;
      ...
      result.setUint8(offset + 1, si2);
      result.setUint8(offset + 2, si3);
      result.setUint8(offset + 3, si4);
    }
  }
