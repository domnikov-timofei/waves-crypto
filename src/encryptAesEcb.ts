...
export function encryptAesEcb(keyInput: Uint8Array, input: Uint8Array) {
  const { encryptionKey } = createAesKeys(keyInput);
  const result = new DataView(input.slice().buffer);
  const t = new DataView(new Uint8Array(128).buffer);
  const a = new DataView(new Uint8Array(128).buffer);

  for (
    let blockOffset = 0;
    blockOffset < result.byteLength;
    blockOffset += 16
  ) {
    for (let i = 0; i < 16; i += 4) {
      t.setUint32(
        i,
        result.getUint32(blockOffset + i, true) ^ encryptionKey.getUint32(i)
      );
    }
    ...
    for (let offset = 4; offset < 56; offset += 4) {
      for (let i = 0; i < 4; i++) {
        a.setUint32(
          i << 2,
          T1[t.getUint8((i << 2) + 3)] ^
            T2[t.getUint8((((i + 1) & 0b11) << 2) + 2)] ^
            T3[t.getUint8((((i + 2) & 0b11) << 2) + 1)] ^
            T4[t.getUint8(((i + 3) & 0b11) << 2)] ^
            encryptionKey.getUint32((offset + i) << 2, true),
          true
        );
      }

      t.setUint32(0, a.getUint32(0));
      t.setUint32(4, a.getUint32(4));
      t.setUint32(8, a.getUint32(8));
      t.setUint32(12, a.getUint32(12));
    }

    for (let i = 0; i < 4; i++) {
      const offset = blockOffset + (i << 2);
      const keyOffset = ((ROUND_COUNT << 2) + i) << 2;

      const key1 = encryptionKey.getUint8(keyOffset + 3);
      const s1 = S[t.getUint8((i << 2) + 3)] ^ key1;

      const key2 = encryptionKey.getUint8(keyOffset + 2);
      const s2 = S[t.getUint8((((i + 1) & 0b11) << 2) + 2)] ^ key2;

      const key3 = encryptionKey.getUint8(keyOffset + 1);
      const s3 = S[t.getUint8((((i + 2) & 0b11) << 2) + 1)] ^ key3;

      const key4 = encryptionKey.getUint8(keyOffset);
      const s4 = S[t.getUint8(((i + 3) & 0b11) << 2)] ^ key4;

      result.setUint8(offset, s1);
      result.setUint8(offset + 1, s2);
      result.setUint8(offset + 2, s3);
      result.setUint8(offset + 3, s4);
    }
  }
