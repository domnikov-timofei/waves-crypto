const roundConstantWords = new Uint8Array([
  0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab,
  0x4d, 0x9a, 0x2f, 0x5e, 0xbc, 0x63, 0xc6, 0x97, 0x35, 0x6a, 0xd4, 0xb3, 0x7d,
  0xfa, 0xef, 0xc5, 0x91,
]);

export const S = new Uint8Array([
...
export function createAesKeys(keyInput: Uint8Array) {
  ...
  const decryptionKey = new DataView(new Uint32Array(KEY_LENGTH).buffer);

  for (let i = 0; i < 32; i += 4) {
    const value = keyInputCopy.getUint32(i, true);

    keyInputCopy.setUint32(i, value);
    encryptionKey.setUint32(i, value);

    const offset = (((ROUND_COUNT - (i >> 4)) << 2) | ((i >> 2) & 0b11)) << 2;
    decryptionKey.setUint32(offset, value);
  }
  ...
  while (kcI < KEY_LENGTH) {
    keyInputCopy.setUint32(
      0,
      keyInputCopy.getUint32(0) ^
        ((S[keyInputCopy.getUint8(30)] ^
    ...
    for (let i = 4; i < 16; i += 4) {
      const value = keyInputCopy.getUint32(i);
      const prevValue = keyInputCopy.getUint32(i - 4);
      keyInputCopy.setUint32(i, value ^ prevValue);
    }

    keyInputCopy.setUint32(
      16,
      keyInputCopy.getUint32(16) ^
    ...
    for (let i = 0; i < 8 && kcI < KEY_LENGTH; i++, kcI++) {
      encryptionKey.setUint32(kcI << 2, keyInputCopy.getUint32(i << 2));

      const offset = (((ROUND_COUNT - (kcI >> 2)) << 2) + (kcI & 0b11)) << 2;
      decryptionKey.setUint32(offset, keyInputCopy.getUint32(i << 2));
    }

    roundConstantWordsIndex += 1;
  }

  for (let i = 16; i < 224; i += 4) {
    decryptionKey.setUint32(
      i,
      U1[decryptionKey.getUint8(i + 3)] ^
